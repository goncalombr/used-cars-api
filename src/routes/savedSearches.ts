...
function headerEmail(req: any): string {
  return String(req.headers["x-user-email"] || "").trim().toLowerCase();
}

async function assertOwnerOr401(
  req: any,
  res: any,
  id: string
): Promise<any | null> {
...
const PostBody = z.object({
  ...
  cadenceMins: z.number().int().positive().max(7 * 24 * 60).default(1440),
});
...
const PatchBody = z.object({
  ...
  cadenceMins: z.number().int().positive().max(7 * 24 * 60).optional(),
});
...
router.get("/debug/columns", async (_req, res) => {
  try {
    ...
    `)) as any[];
    const columns = (rows || []).map((r: any) => String(r.column_name));
    res.json({ columns });
  } catch (e: any) {
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn(
        "[saved-searches] warn: deleteMany(alertEvent) failed (ignoring)",
        e?.message ?? String(e)
      );
    }
...
router.get("/:id/new-count", async (req, res) => {
  try {
    ...
    if (!s.lastCheck) return res.json({ count: 0 });
